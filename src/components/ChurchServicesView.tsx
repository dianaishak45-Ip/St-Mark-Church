  ...
  return (
    <div className="max-w-6xl mx-auto space-y-16 pb-24 lg:pb-0">
      ...
      <div className="text-center space-y-4">
        ...
        <h1 className="arabic-serif text-4xl lg:text-5xl font-bold text-stone-900 leading-tight">خدمات الكنيسة</h1>
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 text-right" dir="rtl">
        {services.map((service, i) => (
          <motion.div
            ...
            viewport={{ once: true }}
            className="group bg-white p-8 rounded-[2.5rem] border border-stone-100 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all space-y-6"
          >
            <div className={`w-16 h-16 ${service.color} rounded-3xl flex items-center justify-center transition-transform group-hover:scale-110`}>
              <service.icon className="w-8 h-8" />
            </div>
            
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-[10px] font-bold px-2 py-1 bg-stone-50 text-stone-400 rounded uppercase tracking-wider arabic-sans">
                  {service.tag}
                </span>
                <span className="text-stone-300 arabic-sans text-xs">{service.subtitle}</span>
              </div>
              <h3 className="arabic-serif text-3xl font-bold text-stone-800">{service.title}</h3>
              <p className="arabic-sans text-stone-500 text-sm leading-relaxed min-h-[3rem]">
                {service.desc}
              </p>
            </div>

            <div className="pt-4 border-t border-stone-50 flex items-center gap-3 text-amber-700">
      ...
      <section className="bg-stone-900 rounded-[3rem] p-8 lg:p-16 text-white overflow-hidden relative">
        <div className="absolute top-0 right-0 w-64 h-64 bg-amber-500/10 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2" />
        
        <div className="relative z-10 grid lg:grid-cols-2 gap-16 items-center">
          <div className="space-y-8 order-2 lg:order-1 text-right" dir="rtl">
            <div className="space-y-4">
              <div className="inline-flex items-center gap-2 px-3 py-1 bg-amber-500/20 text-amber-400 rounded-full text-[10px] font-bold arabic-sans">
                قسم التربية الكنسية
              </div>
              <h2 className="arabic-serif text-4xl lg:text-5xl font-bold">رعاية النشء والشباب</h2>
          ...
          <div className="order-1 lg:order-2">
            <div className="relative rounded-[2.5rem] overflow-hidden aspect-[4/3] lg:aspect-square">
              ...
              <div className="absolute bottom-8 right-8 left-8 text-right">
                <p className="arabic-serif text-2xl font-bold italic">"رَبِّ الوَلَدَ فِي طَرِيقِهِ، فَمَتَى شَاخَ أَيْضًا لاَ يَحِيدُ عَنْهُ."</p>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* Inquiry Footer */}
      <div className="bg-amber-50 rounded-[2.5rem] p-12 text-center space-y-8">
        ...
        <div className="flex flex-wrap justify-center gap-4">
          <button className="px-10 py-4 bg-stone-900 text-white rounded-2xl font-bold arabic-sans hover:bg-stone-800 transition-colors flex items-center gap-2">
            <MessageSquare className="w-5 h-5" />
            <span>تحدث مع أحد الخدام</span>
          </button>
          <button className="px-10 py-4 bg-white text-stone-700 border border-stone-200 rounded-2xl font-bold arabic-sans hover:bg-stone-50 transition-colors">
